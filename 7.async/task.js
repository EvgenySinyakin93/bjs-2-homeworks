...
class AlarmClock {
  ...
  getCurrentFormattedTime() {
    return new Date.parse("HH:MM");
  }

  //п.5
  start(){
    if(this.intervalId){ //если есть значения, то завершаем
        return;
    }
    this.intervalId = newIntervalid(() => {
            this.alarmCollection.forEach(setup => { //перебираем через foreach
                if(setup.time === this.getCurrentFormattedTime && setup.canCall){ //проверяем совпадение time и выполнение звонка
    ...
    //п.8
    clearAlarms(){
        this.intervalId.stop(); //метод остановки интервала
